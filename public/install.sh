#!/usr/bin/env bash
# Thin, stable dispatcher: resolves which install.sh to actually run — the
...
# to change; the real installer lives at the repo root and evolves with the
# project, one release at a time.
set -euo pipefail

REPO="zerobearing2/omatalk"
if [ -n "${OMATALK_REF:-}" ]; then
  url="https://raw.githubusercontent.com/$REPO/$OMATALK_REF/install.sh"
else
