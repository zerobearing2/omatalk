...
DEFAULTS = {
    ...
    # Raw PCM piped to stdin, not a file path: the first Player.play()
    # appends `--rate <rate> --channels 1 -` once per Utterance, so a custom
    # player here must accept the same (only pw-cat --raw does;
    # pw-play/aplay/etc. expect a self-describing container file, not
    # headerless PCM on stdin).
